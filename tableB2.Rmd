...
=======
setwd("C:/Users/lucas/Documents/CMU/3_Junior Year/Fall/Metrics II 73374/__Final Project/374-Final-Project/Admin Stuff/aer_replication/data/19th")
>>>>>>> 317d4c4edba2de8148ec0f560582db9a2de56992
...
```{r Column 4}
#Need to cluster on block, extract SE, and find # of observations
...
B24 <- ivreg(log_rentals_1864 ~ death_ind|dist_urinal + old_sewer + dist_pump, data = temp1) #need to cluster on block SE
summary(B24)

testM <- cluster.robust.se(B24$model, temp1$block)

#Coefficients, Transformed exp(beta)-1, SE
B24$coefficients[2]
B24brack = exp(B24$coef[2])-1

#B24$se[1,1] Need to extract se from ivreg

#Number of Obs, Mean outside BSP, BW

df %>% # Num of Obs broken
  filter(broad == 0 & dist2< 0.292) %>% 
  nrow() 
nrow(temp1)
...
B25 <- ivreg(temp1$log_rentals_1864 ~ temp1$deaths|temp1$dist_urinal + temp1$old_sewer + temp1$dist_pump) #need to cluster on block SE
summary(B25)
...
#B25$se[1,1] Need to extract se from ivreg
