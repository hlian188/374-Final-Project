...
houses$taxexon1853 = ifelse(is.na(houses$tax1_1853), 0, 1)
houses = houses %>%
  select(lnrentals, lntax, taxexon1853, sewer1, sewer2, sewer4, dist_pump, dist_cent, dist_pit_fake, 
...
#we observe that we have a p value < .05, so that 
```

```{r}
#Covariate balance
test = rdrobust(y = houses$dist_cent, x = houses$dist2, vce = "nn", cluster = houses$block)
...
controls = cbind(houses$dist_cent, houses$dist_square, houses$dist_fire, houses$dist_thea, houses$dist_police,
                 ...
                 houses$dist_pit_fake, houses$taxexon1853)
par(mfrow = c(3,5))
rdplot(y = houses$dist_cent, x = houses$dist2, x.lim = c(-1, 1))
rdplot(y = houses$dist_square, x = houses$dist2, x.lim = c(-1, 1))
plots = vector(mode = "list", length = 15)
for (i in 1:15) {
  p = rdplot(y = controls[,i], x = houses$dist2, x.lim = c(-1, 1))$rdplot
  plots[[i]] = p
}
library(gridExtra)
library(tidyverse)
